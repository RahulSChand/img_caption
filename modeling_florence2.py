...
# class Florence2LearnedPositionalEmbedding(nn.Module):
#     """
#     This module learns positional embeddings up to a fixed maximum size.
#     """
...
#         # and adjust num_embeddings appropriately. Other models don't have this hack
#         self.offset = 2
#         self.emb = nn.Embedding(num_embeddings + self.offset, embedding_dim)
...
#         return self.emb(positions + self.offset)
...
# class Florence2ScaledWordEmbedding(nn.Module):
#     """
#     This module overrides nn.Embeddings' forward by multiplying with embeddings scale.
#     """
...
#     def forward(self, input_ids: torch.Tensor):
#         return self.emb(input_ids) * self.embed_scale
...
class Florence2SdpaAttention(Florence2Attention):
    def forward(
        ...
        if (
            is_cross_attention
            and past_key_value is not None
            and past_key_value[0].shape[2] == key_value_states.shape[1] and 2==3
        ):
            # reuse k,v, cross_attentions
        ...
        if self.is_decoder:
            ...
            # can concat previous decoder key/value_states to current projected key/value_states (third "elif" case)
            # if encoder bi-directional self-attention `past_key_value` is always `None`
            # print("this is the shape of key states", key_states.shape)
            # print("this is the shape of value states", value_states.shape)
...
class Florence2DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        #present_key_value = (tensor,tensor)
        ...
        if encoder_hidden_states is not None:
            ...
            # print("is None: ",cross_attn_past_key_value == None)
        ...
        # print(type(present_key_value),len(present_key_value)   )
        # for i in range(4):
        #     print("p_0 ",present_key_value[i][0].shape)
...
class Florence2Decoder(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            # print("decoder input_ids", input_ids)
...
class Florence2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        ...
        print("input decoder id:", decoder_input_ids)
        
        
        # Check and print shapes for non-None tensor arguments
        ...
        #     print("decoder_head_mask shape:", decoder_head_mask.shape)
        # if cross_attn_head_mask is not None:
        #     print("cross_attn_head_mask shape:", cross_attn_head_mask.shape)
        # # if encoder_outputs is not None:
        # #     print("encoder_outputs shape:", [output.shape for output in encoder_outputs])
        # # if past_key_values is not None:
        # #     print("past_key_values shape:", [[tensor.shape for tensor in layer] for layer in past_key_values])
        # if inputs_embeds is not None:
        #     print("inputs_embeds shape:", inputs_embeds.shape)
        # if decoder_inputs_embeds is not None:
        #     print("decoder_inputs_embeds shape:", decoder_inputs_embeds.shape)
        # if labels is not None:
        #     print("labels shape:", labels.shape)


        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        # print(decoder_attention_mask, attention_mask.shape)
        if labels is not None:
            if use_cache:
        ...
        #         print("past_key_values[i]", past_key_values[i][0].shape)
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Print information about returned values
